...
type GreeterHTTPServer interface {
	// SayHelloSends a greeting
	SayHello(context.Context, *HelloRequest) (*HelloReply, error)
}
type GreeterXSignMiddlewareMiddleware middleware.Middleware

func NewGreeterHTTPServerMiddleware(
	XSignMiddleware GreeterXSignMiddlewareMiddleware,
) middleware.Middleware {
	return selector.Server(
		selector.Server(
			middleware.Middleware(XSignMiddleware),
		).Path(OperationGreeterSayHello).Build(),
	).Path(
...
func GenerateGreeterHTTPServerRouteInfo() []route.Route {
	routes := make([]route.Route, 0, 1)
	routes = append(routes, route.Route{
		Method:  "GET",
		Path:    "/helloworld/{name}",
		Comment: "Sends a greeting",
	})
	return routes
}
